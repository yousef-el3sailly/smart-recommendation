  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      <header className="sticky top-0 z-50 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container px-3 sm:px-6 flex h-14 sm:h-16 items-center justify-between gap-1 sm:gap-2">
          <div className="flex items-center gap-1 sm:gap-2 min-w-0">
            <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
              <SheetTrigger asChild>
                <Button variant="ghost" size="icon" className="md:hidden h-10 w-10 shrink-0" aria-label={t('nav.openMenu')}>
            ...
            <Link to="/" className="flex items-center gap-1.5 sm:gap-2 font-bold text-base sm:text-xl min-w-0">
          ...
          <div className="flex items-center gap-0.5 sm:gap-2 shrink-0">
            <LanguageSwitcher />
            <Button variant="ghost" size="icon" className="relative h-10 w-10 sm:h-11 sm:w-11" onClick={() => navigate('/cart')} aria-label={t('nav.cart')}>
              <ShoppingCart className="h-5 w-5 sm:h-6 sm:w-6" />
              {totalItems > 0 && (
                <Badge className="absolute top-0.5 right-0.5 sm:top-1 sm:right-1 h-4 min-w-4 sm:h-5 sm:min-w-5 px-1 flex items-center justify-center p-0 text-[10px] font-bold">
                  {totalItems}
                </Badge>
              )}
            </Button>
            {user ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon" className="h-10 w-10 sm:h-11 sm:w-11" aria-label={t('nav.account')}>
      ...
      <footer className="border-t py-5 sm:py-6 px-4 text-center text-xs sm:text-sm text-muted-foreground">
        {t('footer.copyright')}
      </footer>
    </div>
  );
