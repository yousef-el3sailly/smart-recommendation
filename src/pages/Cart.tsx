                ...
                <CardContent className="p-4 flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.customization && (
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {t('customize.ram')}: <span className="font-medium text-foreground">{item.customization.ram}</span>
                        {' · '}
                        {t('customize.ssd')}: <span className="font-medium text-foreground">{item.customization.ssd}</span>
                      </p>
                    )}
                    <p className="text-lg font-bold text-primary mt-1">{item.unitPrice.toLocaleString()} EGP</p>
                  </div>
                  <div className="flex flex-col items-end justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => updateQuantity(item.key, item.quantity - 1)}>
                  ...
                  {items.map(item => (
                    <div key={item.key} className="flex justify-between text-sm">
                      <span className="truncate me-2">
                        {item.product.name}
                        {item.customization ? ` (${item.customization.ram} / ${item.customization.ssd})` : ''}
                        {' × '}{item.quantity}
                      </span>
