...
const Products = () => {
  ...
  return (
    <UserLayout>
      <SeoHead title={t('products.title')} />
      <div className="container px-4 sm:px-6 py-6 sm:py-8">
        <h1 className="text-2xl sm:text-3xl font-bold mb-5 sm:mb-6">{t('products.title')}</h1>
          ...
          {showFilters && (
            <div className="grid sm:grid-cols-2 gap-6 p-4 border rounded-lg bg-muted/30">
              <div className="space-y-2">
                <Label>{t('products.priceRange', { min: priceRange[0].toLocaleString(), max: priceRange[1].toLocaleString() })}</Label>
                <Slider
                  min={0}
                  max={maxPrice}
                  step={100}
              ...
              <div className="space-y-2">
                <Label>{t('products.minRating', { value: minRating })}</Label>
                <Slider
                  min={0}
                  max={5}
                  step={0.5}
        ...
        <p className="text-sm text-muted-foreground mb-4">{t('products.found', { count: filtered.length })}</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5 lg:gap-6">
            ...
            {filtered.length === 0 && (
              <div className="col-span-full text-center py-16 space-y-2">
                ...
                <p className="text-lg font-semibold">{t('products.noResults')}</p>
                <p className="text-muted-foreground text-sm">{t('products.tryAdjust')}</p>
              </div>
            )}
          </div>
        )}
