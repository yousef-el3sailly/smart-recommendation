                ...
                {confirmedOrder.items.map((it) => (
                  <div key={it.key} className="flex justify-between text-sm">
                    <span className="truncate pe-2">
                      {it.product.name}
                      {it.customization ? ` (${it.customization.ram} / ${it.customization.ssd})` : ''}
                      {' × '}{it.quantity}
                    </span>
                    <span className="whitespace-nowrap">{(it.unitPrice * it.quantity).toLocaleString()} EGP</span>
                  </div>
                ))}
              ...
              {items.map(item => {
                ...
                return (
                  <div key={item.key} className="flex justify-between text-sm gap-2">
                    ...
                    <span className="whitespace-nowrap">{(item.unitPrice * item.quantity).toLocaleString()} EGP</span>
                  </div>
                );
              })}
