...
const USAGE_OPTIONS: { value: Enums<'usage_type'>; label: string }[] = [
...
const PRIORITY_OPTIONS: { value: Enums<'performance_priority'>; label: string }[] = [
...
const Profile = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: 'Profile updated!', description: 'Your recommendations will refresh shortly.' });
    } catch (err: any) {
        ...
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={studyYear} onValueChange={setStudyYear}>
                  <SelectTrigger id="study-year"><SelectValue placeholder="Select year" /></SelectTrigger>
                  <SelectContent>
                    {[1, 2, 3, 4, 5].map((y) => (
            ...
            <Button onClick={handleSave} disabled={saving} className="w-full" size="lg">
              {saving ? (
                <><span className="mr-2 inline-block h-4 w-4 border-2 border-current border-t-transparent rounded-full animate-spin" /> Saving...</>
              ) : 'Save Changes'}
            </Button>
          </CardContent>
        </Card>
