...
const AdminUsers = () => {
  ...
  const updateRole = async (userId: string, newRole: AppRole) => {
    ...
    toast({ title: t('admin.usersPage.roleUpdated') });
  ...
  return (
    <AdminLayout>
      <h2 className="text-2xl font-bold mb-4">{t('admin.usersPage.title')}</h2>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>{t('admin.usersPage.faculty')}</TableHead>
          ...
          <TableBody>
            {isLoading ? (
              <TableRow><TableCell colSpan={5} className="text-center py-8">{t('admin.usersPage.loading')}</TableCell></TableRow>
            ) : filteredProfiles.length === 0 ? (
              <TableRow><TableCell colSpan={5} className="text-center py-8 text-muted-foreground">
                {profiles && profiles.length > 0 ? t('admin.usersPage.noMatch') : t('admin.usersPage.noUsers')}
              </TableCell></TableRow>
            ) : (
              filteredProfiles.map((p) => (
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    <Badge variant="secondary">{p.faculty ?? t('admin.usersPage.other')}</Badge>
                  </TableCell>
                  <TableCell>
                    <Select value={getRoleForUser(p.user_id)} onValueChange={(v) => updateRole(p.user_id, v as AppRole)}>
                      ...
                      <SelectContent>
                        <SelectItem value="user">{t('admin.usersPage.roles.user')}</SelectItem>
